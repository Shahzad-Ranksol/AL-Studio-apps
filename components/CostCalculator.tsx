...
const CostCalculator: React.FC = () => {
  ...
  const analysis = useMemo(() => {
    ...
    // Dynamic Material Prices based on City selection
    ...
    if (inputs.floorFinish === 'Marble') cementFactor += 0.07;
  ...
  const handleGenerateVisuals = async () => {
    ...
    try {
      const elevationPrompt = `Modern Pakistani house elevation, ${inputs.floors}, ${inputs.area} ${inputs.unitType}, ${inputs.quality} style, ${inputs.city} context.`;
      const [elevation, blueprint] = await Promise.all([
        generateArchitecturalImage(elevationPrompt),
        generateArchitecturalImage(`Floor plan layout for ${inputs.area} ${inputs.unitType} plot in Pakistan.`)
      ]);
  ...
  return (
    <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-10 text-slate-900 border border-slate-100 space-y-12">
              ...
              <div>
                ...
                <div className="flex bg-white p-1 rounded-xl border border-slate-200">
                  {['Concrete', 'Tiles', 'Marble'].map(f => (
                    <button key={f} onClick={() => setInputs({...inputs, floorFinish: f as any})} className={`flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${inputs.floorFinish === f ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-slate-600'}`}>{f}</button>
                  ))}
                </div>
              </div>
              <div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="sticky top-24 space-y-6">
            ...
            <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-lg">
              <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">Localized Material Breakdown</h4>
            ...
            <button onClick={handleGenerateVisuals} disabled={isGeneratingVisuals} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-800 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-3 border border-slate-200">
              {isGeneratingVisuals ? <><span className="w-4 h-4 border-2 border-slate-400 border-t-slate-800 rounded-full animate-spin"></span> Generating Visuals</> : <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h7"/><path d="m9 15 3-3 3 3"/><path d="M12 12v9"/></svg> Generate Design Visuals</>}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-emerald-50 rounded-2xl p-6 border border-emerald-100 flex gap-4 items-start">
         <div className="w-10 h-10 bg-emerald-600 rounded-full flex items-center justify-center text-white shrink-0 mt-1"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg></div>
         <div className="space-y-1">
            <h4 className="font-black text-emerald-900 text-sm uppercase">City-Aware Analysis</h4>
            <p className="text-xs text-emerald-700 leading-relaxed">Estimates are adjusted for <span className="font-bold">{inputs.city}</span> market conditions. Steel rates include regional transport from Port Qasim/Local mills. Cement prices are current for brands like Lucky/Maple Leaf in your area.</p>
         </div>
      </div>
    </div>
  );
